...
abstract class WebSocketServer {
  ...
  protected function send($user, $message) {
	  		//file_put_contents("meysam.txt", "meysam websocket 0 - in send - user: ".json_encode($user)." message: ".json_encode($message)." \n", FILE_APPEND);
  ...
  protected function disconnect($socket, $triggerClosed = true, $sockErrNo = null) {
	...
	//file_put_contents("meysam.txt", "in websocket - in disconnect - disconected user id:".json_encode($disconnectedUser->id)." \n", FILE_APPEND);

	//file_put_contents("meysam.txt", "meysam websocket 1 - in disconnect - disconnectedUser: ".json_encode($disconnectedUser)." \n", FILE_APPEND);
	//file_put_contents("meysam.txt", "meysam websocket 2 - in disconnect - socket: ".json_encode($socket)." \n", FILE_APPEND);
	//file_put_contents("meysam.txt", "meysam websocket 3 - in disconnect - user id: ".json_encode($disconnectedUser->id)." \n", FILE_APPEND);

	//file_put_contents("meysam.txt", "in websockets - in disconnect - keys in sockets array: ".json_encode(array_keys($this->sockets))." \n", FILE_APPEND);
	//file_put_contents("meysam.txt", "in websockets - in disconnect - keys in users array: ".json_encode(array_keys($this->users))." \n", FILE_APPEND);
    ...
    if ($disconnectedUser !== null) {
	  ...
	  //file_put_contents("meysam.txt", "in websockets - in disconnect - remaining keys in sockets array: ".json_encode(array_keys($this->sockets))." \n", FILE_APPEND);
	  //file_put_contents("meysam.txt", "in websockets - in disconnect - remaining keys in users array: ".json_encode(array_keys($this->users))." \n", FILE_APPEND);

	  //meysam added
	 
	   //foreach ($this->sockets as $uid=>$uname) {
			  		//file_put_contents("meysam.txt", "meysam hoho - in getUserByName - loop username: ".json_encode($uname)." \n", FILE_APPEND);
	   //}
	   //foreach ($this->users as $uid=>$uname) {
			  		//file_put_contents("meysam.txt", "meysam dodo - in getUserByName - loop username: ".json_encode($uname)." \n", FILE_APPEND);
	  // }
	   ///////////////////
  ...
  //meysam added
  //public function removeSpecificUserNameFromSocketList($uid)
  //{
	  //file_put_contents("meysam.txt", "meysam server removeSpecificUserNameFromSocketList- in connected - uid: ".json_encode($uid)." \n", FILE_APPEND);
